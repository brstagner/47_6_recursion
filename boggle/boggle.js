...
function findPaths(board, letter, y, x) {
  let paths = [];
  if (y < 4 && board[y + 1][x] === letter) {
    paths.push({ y: y + 1, x: x });
  }
  if (x < 4 && board[y][x + 1] === letter) {
    paths.push({ y: y, x: x + 1 });
  }
  if (y > 0 && board[y - 1][x] === letter) {
    paths.push({ y: y - 1, x: x });
  }
  if (x > 0 && board[y][x - 1] === letter) {
    paths.push({ y: y, x: x - 1 });
  }
  return paths;
}

function findNext(board, word, paths) {
  if (!word.length) {
    return true;
  }
  let nextPaths = [];
  for (let path of paths) {
    nextPaths.push(findPaths(board, word[0], path.y, path.x));
  }
  if (!nextPaths.length) {
    return false;
  }
  nextPaths = nextPaths.flat();
  return findNext(board, word.slice(1), nextPaths);
}

function findFirst(board, letter) {
  let paths = [];
  for (let y = 0; y < 5; y++) {
    for (let x = 0; x < 5; x++) {
      if (board[y][x] === letter) {
...
function find(board, word) {
  let paths = findFirst(board, word[0]);
  return findNext(board, word.slice(1), paths);
}
